...
do
    ...
    if [ ${i} != `expr ${TRAIN_STEP} - 1` ]; then
        ...
        python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT \
            $(dirname "$0")/src/test_calc_val_gradient_fasterrcnn.py $CONFIG ${workdir}/latest.pth --gpu-collect \
            --launcher pytorch --out-path=${workdir}

        python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT \
            $(dirname "$0")/src/test_calc_s_test_fasterrcnn.py $CONFIG ${workdir}/latest.pth \
            --options data.train_influ.ann_file=${labeled_file} --launcher pytorch \
            --out-path=${workdir} --in-path=${workdir} --img-num=${S_TEST_IMG_NUM}
        
        python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT \
            $(dirname "$0")/src/test_calc_influ_fasterrcnn.py $CONFIG ${workdir}/latest.pth --gpu-collect \
