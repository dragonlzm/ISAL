...
def setup_env():
    ...
    logging.setup_logging() # output to bash
...
def setup_model():
    ...
    if cfg.MODEL.TYPE[-1] <= "9" and cfg.MODEL.TYPE[-1] >= "0":
        # custom network
        logger.info(logging.dump_log_data(model.get_parameter_number(), "complexity"))
    else:
...
def train_model():
    ...
    for cur_epoch in range(start_epoch, cfg.OPTIM.MAX_EPOCH):
        ...
        # Save a checkpoint
        if (cur_epoch + 1) in cfg.SAVE_EPOCH:
            # Evaluate the model
            test_epoch(test_loader, model, test_meter, last_epoch)
            file = cp.save_checkpoint(model, optimizer, last_epoch, False)
            logger.info("Wrote checkpoint to: {}".format(file))

    # Evaluate the model
    test_epoch(test_loader, model, test_meter, last_epoch)
    # Check if checkpoint is best so far (note: should checkpoint meters as well)
    stats = test_meter.get_epoch_stats(last_epoch)
    best = stats["top1_err"] <= best_err
    best_err = min(stats["top1_err"], best_err)
    # Save a checkpoint
    file = cp.save_checkpoint(model, optimizer, last_epoch, best)
    logger.info("Wrote checkpoint to: {}".format(file))

    cfg.TEST.BEST_TOP1_ERR = best_err
    logger.info("BEST_TOP1_ACC: {}".format(100.0 - best_err))
