...
from pycls.datasets.svhn import SVHN
from torch.utils.data.distributed import DistributedSampler
from torch.utils.data.sampler import RandomSampler

# Supported datasets
_DATASETS = {"cifar10": Cifar10, "cifar100": Cifar100, "imagenet": ImageNet, "svhn": SVHN}

# Default data directory (/path/pycls/pycls/datasets/data)
_DATA_DIR = os.path.join(os.path.dirname(__file__), "data")

# Relative data paths to default data directory
_PATHS = {"cifar10": "cifar10", "cifar100": "cifar100", "imagenet": "imagenet", "svhn": "svhn"}


def _construct_loader(dataset_name, split, batch_size, shuffle, drop_last, ouput_id=False):
    ...
    if cfg.TRAIN.DATAPATH:
        data_path = cfg.TRAIN.DATAPATH
    elif cfg.TEST.DATAPATH:
        data_path = cfg.TEST.DATAPATH
    else:
        data_path = os.path.join(_DATA_DIR, _PATHS[dataset_name])
    # Construct the dataset
    dataset = _DATASETS[dataset_name](data_path, split, ouput_id)
...
def construct_test_loader(ouput_id=False):
    """Test loader wrapper."""
    return _construct_loader(
