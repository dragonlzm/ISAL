...
def parse_args():
    ...
    parser.add_argument('--in-path', type=str, help='the path to load the gradient on validation set')
...
def main():
    ...
    val_sum_grad = [torch.load(os.path.join(args.in_path,file[1])).cuda() for file in all_pt_file]
    ...
    influence_list, idxes_list = calc_influence_funtion(model, test_loader, params, val_sum_grad)
