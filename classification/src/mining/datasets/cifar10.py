...
class Cifar10():
    ...
    def __init__(self, data_dir, work_step_dir, initial_ratio=None, query_indices=None, prev_labeled_file=None, prev_unlabeled_file=None):
        ...
        self._test_data_path = os.path.join(self.data_path, "test_batch")
    ...
    def load_dataset_init(self):
        """load cifar10 training data, indices it and save as one pickle
        return:
            the num of dataset sample
        """
        # Compute data batch names
        batch_paths = glob.glob(os.path.join(self.data_path, "data_batch_*"))
        # Load data batches
        inputs, labels = [], []
