...
do
    ...
    rm -rf ${workdir_step}/checkpoints
    ...
    if [ ${i} != `expr ${#RATIOS[@]} - 1` ]; then
        python $(dirname "$0")/src/test.py --config-path=${CONFIG_PATH} \
            --work-dir=${workdir_step} \
            --test-data-file=${unlabeled_file} --model-path=${workdir_step}/'model.pth' # get trained model which get best acc on test dataset
    fi
done
